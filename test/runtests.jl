...
@testset "CSP.Policy" begin 
    ...
    @test isnothing(policy.sandbox)
    @test isnothing(policy.trusted_types)

    for prop in fieldnames(Policy)
        http_name = replace(string(prop), "_"=>"-")
        policy[http_name] = getproperty(policy, prop)
        @test policy[http_name] == getproperty(policy, prop)
    end
end
...
@testset "CSP.headers" begin
    ...
    csp_header = first(CSP.headers(policy))
